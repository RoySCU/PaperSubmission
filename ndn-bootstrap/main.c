...
int main(void)
{
    ...
    //pid = thread_create(_stack, sizeof(_stack),
                            //THREAD_PRIORITY_MAIN - 1, THREAD_CREATE_STACKTEST, ndn_bootstrap, &key, "boot");

    //_send.content.ptr = _reply.content.ptr;
    //_send.type = NFL_START_BOOTSTRAP;

    //msg_send_receive(&_send, &_reply, pid);

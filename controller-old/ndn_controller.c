...
static int on_certificate_request(ndn_block_t* interest)
{
    ...
    char* id = "/ty_device";
    ndn_shared_block_t* id_ptr = ndn_name_from_uri(id, strlen(id));
    ...
    ndn_shared_block_t* cert_packet =
    ...
    if (ndn_app_put_data(handle, cert_packet) != 0) {
        DPRINT("Controller (pid=%" PRIkernel_pid "): cannot put Ceritificate Response\n",
               handle->id);
        return NDN_APP_ERROR;
    }

    ndn_shared_block_release(id_ptr);
    ndn_shared_block_release(signed_cert);
    //ndn_shared_block_release(cert_packet);
...
static int on_bootstrap_request(ndn_block_t* interest)
{
  // /ndn/sign-on/{digest of BKpub}/{ECDSA signature by BKpri}
    ...
    */
    ...
    //ndn_shared_block_release(big_packet);
    free(big_buf);
